...
const schema = {
    newLogin : Joi.object({
        login_email : Joi.string().email({ minDomainSegments: 2, tlds: { allow: ['com', 'net', 'fr', 'io'] } }).required().messages({
            'string.email': `Oyez oyez veuillez mettre un e-mail ou password valide !`,
            'string.empty': `Si rejoindre le coaching de la force est votre volonté, il faut saisir de ton e-mail !`,
        ...
        login_password : Joi.string().required().messages({
            'string.empty': `Si rejoindre le coaching de la force est votre volonté, il faut saisir de ton password !`,
